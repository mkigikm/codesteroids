...
var Ship = Asteroids.Ship = function (pos) {
  ...
  this.guidance = new Asteroids.Guidance(Ship.LIMITS,
      new Asteroids.Vector({x:0, y:-1}));
...
Ship.prototype.fireBullets = function () {
  if (this.bulletTimer.off() && this.invinsibleTimer.off()) {
    ...
    return _.map([-Math.PI / 20, -Math.PI / 40, 0, Math.PI / 20, Math.PI / 40], function (rotate) {
      return new Asteroids.Bullet(this, rotate);
    }, this).concat([new Asteroids.GuidedBullet(this)]);
  }
...
Ship.prototype.draw = function (ctx) {
  ...
  ctx.fill();
  ctx.restore();
  ctx.drawImage(Asteroids.PARTY_HAT, this.pos.x-5, this.pos.y-25,
    12, 20);
};
