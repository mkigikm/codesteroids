...
RotChar.prototype.orbiter = function (ship) {
  switch (this.formation.personality.state) {
    case "seeking":
      if (this.formation.center.dist(this.formation.goal) > 100) {
        this.head.set(this.formation.goal.subC(this.formation.center)).norm();
        break;
      }
...
RotChar.prototype.sentinal = function (ship) {
  switch (this.formation.personality.state) {
    case "seeking":
      if (this.formation.center.dist(this.formation.personality.goal) > 10) {
        this.head.set(this.formation.personality.goal.subC(this.formation.center)).norm();
        break;
      }
      this.formation.personality.state = "patrolling";
    case "patrolling":
      if (this.formation.center.dist(this.formation.personality.goal) > 300) {
        this.formation.personality.state = "seeking";
        break;
      } else if (this.formation.center.dist(this.formation.personality.goal) > 200) {
