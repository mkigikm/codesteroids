...
RotChar.prototype.draw = function (ctx) {
  ...
  if (this.coordinator() && this.personality.goal) {
    ctx.fillStyle = this.personality.update === RotChar.prototype.orbiter ? 'purple' : 'blue';
    ctx.beginPath();
...
RotChar.prototype.fireShot = function (ship) {
  if (Math.random() < 0.000) {
    return new Asteroids.ProjectileChar(this.pos, ";", ship.pos);
  }
};
})();
