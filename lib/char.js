...
Char.prototype.initFormation = function () {
  ...
  this.rightMost = this.rightMost || this.findEnd("right");

  if (this.guidance.center) {
    this.guidance.center.set(this.pos.midPoint(this.rightMost.pos));
  } else {
    this.choosePersonality();
    this.guidance.center = this.pos.midPoint(this.rightMost.pos);
    this.initNeighbors();
  }
};
